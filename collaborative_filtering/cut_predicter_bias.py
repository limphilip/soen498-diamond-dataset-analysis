...
# -------------------------------------------------
# Evaluate the prediction model without bias
# -------------------------------------------------
...
# Add a bias
# -------------------------------------------------
minMaxPrice = predictions.agg(min('price'), max('price')).collect()[0]
minPrice = float(minMaxPrice['min(price)'])
maxPrice = float(minMaxPrice['max(price)'])

nPriceCol = ((col("price") - minPrice) / (maxPrice - minPrice)).cast(DoubleType())
normalizedPriceDF = predictions.withColumn("nPrice", nPriceCol)

finalPredictionCol = when(col("nPrice") < 0.5, col("prediction") - col("nPrice")) \
			.when((col("nPrice") >= 0.5) & (col("prediction") + col("nPrice") > 5), 5.0) \
			.otherwise(col("prediction") + col("nPrice"))
normalizedPriceDF = normalizedPriceDF.withColumn("finalPrediction", finalPredictionCol)
# normalizedPriceDF.orderBy(desc("price")).show(15)

finalEvaluator = RegressionEvaluator(metricName="rmse", labelCol="cut", predictionCol="finalPrediction")
finalRMSE = finalEvaluator.evaluate(normalizedPriceDF)

# -------------------------------------------------
# Print
# -------------------------------------------------
# Print the rmse and the percentage of accuracy after rounding
print(str(rmse) + "," + str(percentError) + "," + str(finalRMSE), end="")
